...
def bilinear_sampler_1d_h_Softsplat(input_images, x_offset, metric, wrap_mode='border', name='bilinear_sampler', **kwargs):
    def _repeat(x, n_repeats):

        rep = x.view(-1,1).repeat(1, n_repeats)
        return rep.reshape(-1)

    def _transform(input_images, x_offset, metric):
        ...
        x_t_flat = x_offset.view(-1) * _width_f
        x_t = (x_t_flat.reshape(1, _height, _width))
        y_t = (0*y_t_flat.reshape(1, _height, _width))
        # import IPython;IPython.embed()
        tenFlow = torch.cat([x_t, y_t], 0)
        tenMetric = 50*x_offset

        input_transformed = softsplat.FunctionSoftsplat(tenInput=input_images, tenFlow=tenFlow.unsqueeze(0), tenMetric= tenMetric, strType='softmax')
        input_transformed[torch.isnan(input_transformed)] = 0
        input_transformed[input_transformed==0] = 0.5
        # import torchvision
        # torchvision.utils.save_image(input_transformed[0], 'images_save/test_splat.png')
        # torchvision.utils.save_image(input_images[0], 'images_save/test_splat_or.png')
        # torchvision.utils.save_image(input_transformed[0]-input_images[0], 'images_save/test_splat_diff.png')
        # torchvision.utils.save_image(metric[0]/80., 'images_save/test_splat_depth.png')
        # import IPython;IPython.embed()
    ...
    output = _transform(input_images, x_offset, metric)
    return output
